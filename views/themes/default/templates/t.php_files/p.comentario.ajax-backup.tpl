{if $tsComments.num > 0}
	{foreach from=$tsComments.data item=c}
		<div id="div_cmnt_{$c.cid}" class="especial{if $tsPost.autor == $c.c_user}1{elseif $c.c_user == $tsUser->uid}3{/if}">
         ...
         <div class="comentario-post clearbeta">
            ...
            <div class="avatar-box" style="z-index: 99;">
               <a href="{$tsConfig.url}/perfil/{$c.user_name}">
						<img width="48" height="48" title="Avatar de {$c.user_name} en {$tsConfig.titulo}" src="{$c.c_avatar}" class="avatar-48 lazy" style="position: relative; z-index: 1; display: inline;">
					</a>
            ...
            <div class="comment-box has-opacity" id="pp_{$c.cid}" {if $c.c_status == 1 || !$c.user_activo && $tsUser->is_admod}style="level:0.5"{/if} >
