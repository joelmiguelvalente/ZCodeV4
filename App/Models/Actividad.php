...
class Actividad
{
    ...
    public function __construct(Core $Core, User $User, Avatar $Avatar)
    {
        $this->Core = $Core;
        $this->User = $User;
        $this->Avatar = $Avatar;
    ...
    */
    public function delActividad()
    {
        ...
        # COMPROBAMOS
        if ($data['user_id'] !== $this->Core->uid) {
            return '0: No puedes borrar esta actividad.';
        }
        if (db_exec([__FILE__, __LINE__], 'query', "DELETE FROM @actividad WHERE ac_id = $ac_id")) {
    ...
    */
    ...
    {
        return $this->createLink('post', $data['post_id'], $param);
    }

    private function linkMonitorOfFoto(array $data = [], string $param = '')
    {
        return $this->createLink('foto', $data['foto_id'], $param);
    }
    /**
     * @name makeOracion
     * @access private
    ...
    private function makeOracion($data = null)
    {
        ...
        $oracion['uid'] = $this->Avatar->loadAvatar((int)$data['user_id'], 'use');
        ...
        switch ($ac_type) {
            ...
            case 8:
                // AVATARES
                $img_uno = '<img class="avatar avatar-1" src="' . $this->Avatar->loadAvatar((int)$data['user_id'], 'use') . '"/>';
                $img_dos = '<img class="avatar avatar-1" src="' . $this->Avatar->loadAvatar((int)$data['avatar'], 'use') . '"/>';
                // ORACION
