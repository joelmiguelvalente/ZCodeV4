...
{foreach from=$tsComments.data item=c}
	<div class="ufiItem" id="cmt_{$c.cid}">
	 	<div class="clearfix">
		  	<a href="{$tsConfig.url}/perfil/{$c.user_name}" class="autorPic">
		  		<img alt="{$c.user_name}" src="{$tsConfig.url}/files/avatar/{$c.c_user}_50.jpg" width="32" height="32"/>
		  	</a>
		  	{if $tsComments.user == $tsUser->uid || $c.c_user == $tsUser->uid}
