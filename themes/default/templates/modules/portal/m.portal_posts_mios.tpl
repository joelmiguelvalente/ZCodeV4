                    ...
                    <div class="body">
                    	<ul>
                            ...
                            <li class="categoriaPost" style="background-image:url({$tsConfig.images}/icons/cat/{$p.c_img})">
                                ...
                                <span>Hace {$p.post_date|hace} &raquo; <a href="{$tsConfig.url}/perfil/{$p.user_name}"><strong>{$p.user_name|verificado}</strong></a> &middot; Puntos <strong>{$p.post_puntos}</strong> &middot; Comentarios <strong>{$p.post_comments}</strong></span>
