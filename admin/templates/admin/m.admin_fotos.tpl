								...
								<div class="empty hero">No hay fotos.</div>
								{else}
									...
									<tbody>
										...
										<tr id="foto_{$f.foto_id}">
											...
											<td class="admin_actions">
												<a href="{$tsConfig.url}/fotos/editar.php?id={$f.foto_id}" target="_blank"><img src="{$tsConfig.public}/images/icons/editar.png" title="Editar Foto" /></a>
												<a {if $f.f_status != 2}onclick="admin.fotos.setOpenClosed({$f.foto_id}); return false;"{/if}><img src="{$tsConfig.public}/images/icons/comment.png" title="{if $f.f_status == 2}No disponible{else}Abrir/Cerrar Comentarios{/if}" /></a>
												<a {if $f.f_status != 2}onclick="admin.fotos.setShowHide({$f.foto_id}); return false;"{/if}><img src="{$tsConfig.images}/reactivar.png" title="{if $f.f_status == 2}No disponible{else}Mostrar/Ocultar Foto{/if}" /></a>
											   	<a href="#" onclick="admin.fotos.borrar({$f.foto_id}); return false;"><img src="{$tsConfig.public}/images/icons/close.png" title="Borrar Foto" /></a>
											</td>
										</tr>
										{/foreach}
									</tbody>
