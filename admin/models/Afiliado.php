...
class Afiliado
{
    ...
    public function newAfiliado()
    {
        ...
        if (!$dataIn['titulo'] || !$dataIn['url'] || $dataIn['url'] == 'http://' || !$dataIn['banner'] || $dataIn['banner'] == 'http://' || !$dataIn['descripcion']) {
            die('2: Faltan datos');
        }
        // FILTRAMOS URL
    ...
    public function deleteAfiliado(int $aid = 0)
    {
        ...
        if ($tsUser->is_admod == 1) {
            if (removeDataById([__FILE__, __LINE__], '@afiliados', "aid = $aid")) {
                return '1: Afiliado eliminado';
            }
        } else {
            return '0: T&uacute;o, no puedes hacer eso';
        }
    }
